...
const fnkAppRunner = path.resolve(projectRoot, "..", "DS", "AppRunner.ino");
...
test("FNK0104 compatibility contract matches sibling AppRunner when available", async (t) => {
  const compatibility = JSON.parse(await readFile(projectCompatibility, "utf8"));
  let source;
  try {
    source = await readFile(fnkAppRunner, "utf8");
  } catch (error) {
    if (error.code === "ENOENT") {
      t.skip("sibling DS checkout is not available");
      return;
    }
    throw error;
  }
...
test("counts metadata and comments toward the M5Cardputer line limit", async (t) => {
  const compatibility = JSON.parse(await readFile(projectCompatibility, "utf8"));
  const limit = compatibility.boards.m5cardputer.limits.lines;
  const body = Array.from({ length: limit }, (_, index) => `# comment ${index}`).join("\n");
  const fixture = await makeFixture(t, {
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      error.message.includes(`physical lines exceed m5cardputer limit ${limit}`),
  );
});
