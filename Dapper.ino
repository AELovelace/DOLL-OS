...
static bool dapperFetchToFile(const String& url, const char* destination,
    ...
    ledPulseNetwork();
    //These objects and the transfer buffer are deliberately heap-backed. The
    //Cardputer's Arduino loop task has an 8 KB stack; keeping the TLS/HTTP
    //clients plus a 1 KB buffer there can trip its stack canary as soon as a
    //network-backed Dapper command starts.
...
static String dapperPromptInstallTarget(const DapperRecord& record, const String& currentPath) {
    ...
    while (true) {
        M5Cardputer.update();
        bool submitted = readKeyboard(answer);
        dapperServiceUi();
        drawCommandBar(prompt, answer);
