...
// esp32 configuration
	...
	// DOLL-OS: serve the SD card, which on the Cardputer is wired to SPI and mounted with
	// the standard SD library (storage.ino) -- hence STORAGE_SD, where DS's board uses the
	// S3's SDMMC peripheral and sets STORAGE_SD_MMC.
	//
	// This is why this copy of the library exists at all. FtpServer.cpp is its own
	// translation unit, so this cannot be overridden by a #define in the sketch's .ino --
	// it has to be set in the library config, and DS needs the opposite value in the same
	// place. Two sketches, one shared sketchbook, one macro: the only way both build is for
	// one of them to carry its own copy. sketch.yaml's `dir:` entry is what selects this one.
	// See docs/PORT-FROM-DS.md, Phase 4.
