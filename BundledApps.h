...
than quietly reading zero. All arrays share a pool of 4096 cells.
...
leaves every other DOLL-OS screen also leaves yours. Ctrl+X or Fn+Q aborts the
running app, from KEY, WAIT, or a blocked INPUT.
...
scaled to fill the whole 240x135 panel, with a tiny font for dense grids. CLS
blanks the grid while a canvas is up.
...
has no PSRAM, so the caps are reduced:
