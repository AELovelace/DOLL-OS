...
larger PSRAM-backed resource limits or a layout that does not fit the Cardputer.
...
| Lines | 1,200 | 4,000 |
| Labels | 192 | 256 |
| Numeric variables | 64 | 64 |
| String variables | 32 | 32 |
| String length | 128 | 4,096 |
| Arrays | 16 | 16 |
| Shared array cells | 4,096 | 8,192 |
| Nested `GOSUB` calls | 64 | 64 |
| Canvas size | 40 x 22 | 120 x 60 |
| Non-yielding step guard | 250,000 | 1,000,000 |

Board compatibility covers resource differences that API SemVer alone cannot
express. For example, a 1,500-line app cannot declare `m5cardputer` even if it
uses only opcodes supported by both boards.
...
```text
apps: huge-demo 1.0.0 supports fnk0104; this device is m5cardputer
apps: future-demo requires AppRunner >=2.0.0; installed runtime is 1.3.0
...
## 10. Implemented stages

### Stage 1: Freeze and validate the format (complete)
...
### Stage 2: Read-only repository client (complete)
...
### Stage 3: Transactional package operations (complete)
...
### Stage 4: Bundled-app overlay and publishing (firmware and tooling complete)
